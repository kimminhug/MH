 ...
 * </pre>
 */
public class CmnCBOXTag extends TagSupport {
	private static final long serialVersionUID = 1L;
	
	private String idName;
	private String tabIndex;
	private String etcColVal;
	...
	public String getEtcColVal() {
		return etcColVal;
	}
	public void setEtcColVal(String etcColVal) {
		this.etcColVal = etcColVal;
	}
	...
	public String cmnCode() throws Exception {
		...
		strTag += tagAllOption;
		strTag += tagAddOption;
		strTag += "<input type='hidden' name='"+idName+"' id='"+idName+"' value='' />";
		
		String scriptData = "";
		for(int i=0; i<list.size(); i++) {
			Map map = (Map) list.get(i);

			String attrSelected = StringUtil.nullValue(map.get("CODE")).equals(initVal)?"checked=checked":"";
			strTag += "<label class='radio_input'>";
			strTag += "<input type='checkbox' class='"+idName+"' "
					+ "key=\'"+StringUtil.nullValue(map.get("KEYCOL"))+"\' "
					+ "name='"+idName+"___"+i+"' "
					+ "value=\'"+StringUtil.nullValue(map.get("CODE"))+"\' "+attrSelected+" "
					+ "onchange='fnCboxChgVal(this, \""+idName+"\")'; etcVal='"+StringUtil.nullValue(etcColVal)+"' />";
			strTag += "<span>"+StringUtil.nullValue(map.get("NAME"))+"</span>";
			strTag += "</label>";
		}
		
		if(etcColVal != null){
			strTag += "<input type='text' name='"+idName+"_etc' id='"+idName+"_etc' value='' style='display:none;' />";
		}
