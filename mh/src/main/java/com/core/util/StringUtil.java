...
public class StringUtil{
	...
	/*
	 * 3번째 인자를 감싸서 string을  배열로 리턴
	 * */
	public static String[] splite(String st, String key, String s){
		StringTokenizer token = new StringTokenizer(st, key);
		int tokenCount = token.countTokens();
		String[] arr = new String[tokenCount];
		for(int i = 0; i < tokenCount; i++){
			arr[i] = s + token.nextToken() + s;
		}
		return arr;		
	}
...
/**
     ...
     *</pre>
     */
    public static long zeroConvertForLong(Object obj) {
    	String src = StringUtil.nullValue(obj);
    ...
    public static String passwordRandomValue(String type, int cnt) {
	...
	/**
     * BASE64 인코딩	한글깨짐현상으로 인해 URLEncoder로 한번 더 인코딩
     *
     * @param strPlainText 인코딩 할 문자열
     * @return 인코딩 된 문자열
     */
    public static String Base64URLEncode(String strPlainText) {
        //System.out.println("Base64URLEncode 이전 ================= "+strPlainText);
    	
    	if (strPlainText == null || strPlainText.length() == 0) return "";
        
    	try {
    		
    		strPlainText = URLEncoder.encode(strPlainText, "UTF-8");
        ...
        try {
            ...
            //System.out.println("인코딩 replace 전 == " + new sun.misc.BASE64Encoder().encode(strBuff));
            //return new sun.misc.BASE64Encoder().encode(strBuff);
            return new sun.misc.BASE64Encoder().encode(strBuff).replaceAll("\"","º").replaceAll("\'", "ª").replaceAll("=", "ˇ");
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
     ...
     * @return 디코딩 된 문자열
     */
    public static String Base64URLDecode(String strEncodedText) {
        if (strEncodedText == null || strEncodedText.length() == 0) return "";

        try {
        	strEncodedText = strEncodedText.replaceAll("º","\"").replaceAll("ª", "\'").replaceAll("ˇ", "=");
        	String de = new String(new sun.misc.BASE64Decoder().decodeBuffer(strEncodedText));
        	
        	de = URLDecoder.decode(de, "UTF-8");
