...
public class BackupJobListener implements JobListener {
	...
	public void jobToBeExecuted(JobExecutionContext jobContext) {
		...
		try {
			...
			LOGGER.error("(FdlException)백업작업ID : {}, 배치결과저장(insert) 에러 : {}", backupResult.getBackupOpertId(), e.getMessage());
		} catch (Exception e) {
	...
	public void jobWasExecuted(JobExecutionContext jobContext, JobExecutionException jee) {
		...
		try {
			...
			//LOGGER.error("(NullPointerException)백업결과ID : {}, 백업작업ID : {}, 배치결과저장(update) 에러 : {}", backupResult.getBackupResultId(), backupResult.getBackupOpertId(), e.getMessage());
		} catch (Exception e) {
			//2017.02.13 	이정은 	시큐어코딩(ES)-부적절한 예외 처리[CWE-253, CWE-440, CWE-754]
